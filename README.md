...
Датасет должен быть заранее разбит на три подпапки: `train/`, `val/`, `test`. Внутри каждой подпапки находятся каталоги `images/` (файлы `*.png`) и `labels/` (файлы `*.txt` с тем же именем). В файле разметки каждая строка описывает один объект:
...
1. Используйте готовый ноутбук `faster_rcnn_colab_notebook.ipynb` (загрузите его в Colab или откройте напрямую в Google Drive) и выполните ячейки последовательно.
...
3. Смонтируйте Google Drive и укажите путь к каталогу с данными (корень, где лежат `train/`, `val/`, `test`):
...
* При желании можно менять гиперпараметры обучения (lr, batch_size, num_epochs) и путь сохранения результатов.
